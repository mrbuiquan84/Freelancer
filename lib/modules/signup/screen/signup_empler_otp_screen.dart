...
class SignupEmplerOTPScreen extends StatelessWidget {
  const SignupEmplerOTPScreen({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener(
      bloc: _signupBloc,
      listener: (_, state) {
        if (state is SignupEmplerReceivedOTPState) {
          _token = state.token;
      ...
      child: OTPScreen(
        ...
        onSubmitButtonPressed: (otp) {
          _signupBloc.activeEmpler(otp: otp, token: _token);
        },
      ),
    );
  }
